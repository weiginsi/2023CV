...
class poseDetector():
    ...
    def FindPose(self, img, draw=True):
        ...
        if self.results.pose_landmarks:
            ...
            if draw:
                # 在图片中画出骨骼点并连接起来
                # self.mpDraw.DrawingSpec(color=(0,0,255),circle_radius=5)和self.mpDraw.DrawingSpec(color=(0,255,0)分别定义骨骼点的颜色大小和连接线的颜色大小
...
#测试用主函数
def main():
    cap = cv2.VideoCapture("D:/1_user/videos/5_8/test6.mp4")
    ...
    while True:
        ...
        if flag == 1:
            time.sleep(5)
            flag = 2
       # cv2.imwrite("D:/1_user/videos/5_8/test2_pose.jpg", img)
